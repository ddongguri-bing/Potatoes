    ...
    childDocs.forEach((childDoc) => {
      ...
      const html = `
        <div class="doc__childDoc">
            <img src="/Potatoes/assets/file-icon.svg" class="icon" />
    ...
    docs.forEach((doc) => {
      ...
      listItem.innerHTML = `
          <div class="flex relative">
            <img src="/Potatoes/assets/toggle-icon.svg" alt="토글 아이콘" class="toggle-icon" />
            <a href="/documents/${pathname}" class="doc-item" data-id="${id}">${title}</a>
            <button class="doc-item__add">
              <img src="/Potatoes/assets/plus-icon.svg" alt="새 페이지 추가 버튼" class="icon" />
            </button>
          </div>
        `;
